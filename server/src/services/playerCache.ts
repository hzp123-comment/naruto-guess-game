  ...
  refreshPromise = (async () => {
    ...
    while (appliedGeneration !== refreshGeneration) {
      ...
      searchablePlayers = allPlayers.map((player) => {
        let familyOrgStr = '';
        try {
          const parsed = JSON.parse(player.family_org);
          familyOrgStr = Array.isArray(parsed) ? parsed.join(' ') : String(player.family_org);
        } catch {
          familyOrgStr = String(player.family_org);
        }
        return {
          player,
          search: normalizeSearch(`${player.nickname}\0${player.village}\0${familyOrgStr}`),
        };
      });
      publicList = {
