...
function answerView(target: Player) {
  // 解析 family_org 字段，确保返回数组
  let familyOrg: string[] = [];
  try {
    familyOrg = typeof target.family_org === 'string' 
      ? JSON.parse(target.family_org) 
      : target.family_org || [];
    if (!Array.isArray(familyOrg)) familyOrg = [];
  } catch {
    familyOrg = [];
  }
  ...
  return {
    ...
    eyeTechnique: target.eye_technique,
    hasKekkei: target.has_kekkei,
    isJinchuriki: target.is_jinchuriki,
  };
}
...
async function settleGame(game: SingleGameState, status: 'won' | 'lost'): Promise<boolean> {
  let shouldPersist = true; // 默认允许持久化
  try {
    shouldPersist = await shouldPersistSingleSettlement(game.identityKey, game.id);
  } catch {
    // Redis 不可用时，默认允许持久化
    shouldPersist = true;
  }
